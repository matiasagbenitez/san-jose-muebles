...
export class CreateEstimateDTO {
    ...
    static create(object: { [key: string]: any }): [string?, CreateEstimateDTO?] {
        const { id_project, gen_date, val_date, client_name, title, id_currency, subtotal, discount, fees, total, guarantee, observations, items, id_user } = object;
        ...
        if (items.length === 0) return ['La lista de ítems no puede estar vacía'];
