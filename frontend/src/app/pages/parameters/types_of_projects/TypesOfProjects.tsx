...
import { TypesOfProjectsForm } from ".";
...
export interface TypeOfProjectFormInterface {
  name: string;
  description: string;
}

const initialForm: TypeOfProjectFormInterface = {
  name: "",
  description: "",
};
...
export const TypesOfProjects = () => {
  ...
  const endpoint = "/types_of_projects";
  ...
  const handleSubmit = async (formData: TypeOfProjectFormInterface) => {
    try {
      if (editingId) {
  ...
  return (
    <div>
      <FilterByName
        placeholder="Buscar por nombre de tipo de proyecto"
        state={state}
        dispatch={dispatch}
