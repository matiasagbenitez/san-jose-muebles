...
export const InventoryCategories = () => {
  ...
  const endpoint = "/inventory_categories";
  ...
  const handleDelete = async (row: DataRow) => {
    const confirmation = await SweetAlert2.confirm(
      "¿Eliminar la categoría de herramienta " + row.name + "?"
    );
    try {
  ...
  return (
    <div>
      <FilterByName
        state={state}
        dispatch={dispatch}
        placeholder="Buscar por nombre de categoría de herramienta"
