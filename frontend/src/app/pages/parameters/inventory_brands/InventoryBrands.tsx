...
export const InventoryBrands = () => {
  ...
  const endpoint = "/inventory_brands";
  ...
  const handleDelete = async (row: DataRow) => {
    const confirmation = await SweetAlert2.confirm(
      "¿Eliminar la marca de herramienta " + row.name + "?"
    );
    try {
  ...
  return (
    <div>
      <FilterByName
        state={state}
        dispatch={dispatch}
        placeholder="Buscar por nombre de marca de herramienta"
